...
const useSSEConnection = (symbol, onDataReceived) => {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const eventSource = new EventSource(`/api/stocks-backend/14?symbol=${symbol}`);
...
const FinancialChart = ({ symbol = 'AAPL', chartType = 'candlestick', title }) => {
  const chartRef = useRef(null);
  const chartInstanceRef = useRef(null);
  const { data, loading, error, fetchInitialData, setData } = useFinancialData(symbol);
  ...
  const { connectionStatus, connect, disconnect } = useSSEConnection(symbol, handleRealtimeData);
  ...
  useEffect(() => {
    if (chartInstanceRef.current && data.length > 0) {
      const config = chartType === 'candlestick' 
        ? createCandlestickConfig(data, title || `${symbol} - Candlestick Chart`)
        : createLineConfig(realtimeData.length > 0 ? realtimeData : data, title || `${symbol} - Price Chart`);
  ...
  const previousPrice = data.length > 1 ? data[data.length - 2][2] : currentPrice;
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Financial Data Dashboard</h1>
