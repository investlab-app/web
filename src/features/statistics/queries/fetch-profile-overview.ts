...
const _generateFakeProfileOverview = (): ProfileSummary => {
  return {
    ...
    total_account_value: parseFloat((Math.random() * 10000).toFixed(2)),
    gain: parseFloat((Math.random() * 1000 - 500).toFixed(2)),
    gain_percent: parseFloat((Math.random() * 100 - 50).toFixed(2)),
  };
};
