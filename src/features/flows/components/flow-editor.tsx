...
export function FlowsEditor() {
  ...
  const screenToFlowFns = useRef<
    Record<string, (pos: XYPosition) => XYPosition>
  >({});
  ...
  const flows: Array<FlowCanvasConfig> = [
    {
      ...
      colorClass: 'bg-[var(--color-triggers)]',
    },
    {
      id: '2',
      nodeTypes: {
    ...
    },
    {
      ...
      colorClass: 'bg-[var(--color-actions)]',
    },
  ];
  ...
  return (
    <DnDProvider>
      <div className="flex">
        <div className="flex w-4/5 w-full h-[500px] gap-4">
          {flows.map((flow) => (
            <FlowCanvas
              key={flow.id}
              {...flow}
